...
@interface SScanViewController ()<UINavigationControllerDelegate, UIImagePickerControllerDelegate, AVCaptureMetadataOutputObjectsDelegate, UIGestureRecognizerDelegate>

@property (nonatomic, strong) SScanView *scanRectView;
...
@implementation SScanViewController{
    NSString *appName;
    BOOL delayQRAction;
    BOOL delayBarAction;
}
...
//绘制扫描区域
- (void)drawScanView {
    _scanRectView = [[SScanView alloc] initWithFrame:self.view.frame style:@""];
...
+ (void)recognizeQrCodeImage:(UIImage *)image onFinish:(void (^)(NSString *result))finish {
    [[[SScanViewController alloc] init] recognizeQrCodeImage:image onFinish:finish];
}
