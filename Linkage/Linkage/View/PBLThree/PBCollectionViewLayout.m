...
#pragma mark - 默认参数
static const CGFloat PBDefaultColumnCount = 3;                           ///< 默认列数
static const CGFloat PBDefaultColumnMargin = 10;                         ///< 默认列边距
static const CGFloat PBDefaultRowMargin = 10;                            ///< 默认行边距
static const UIEdgeInsets PBDefaultUIEdgeInsets = {10, 10, 10, 10};      ///< 默认collectionView边距
...
- (void)prepareLayout
{
    ...
    //当列高度数组为空时，即为第一行计算，每一列的基础高度加上collection的边框的top的值
    if (!self.columnHeights.count) {
        for (NSInteger i = 0; i < self.columnCount ;i ++ ) {
            [self.columnHeights addObject:@(self.edgeInsets.top)] ;
        }
    }
    //便利所有的cell，计算所有cell的布局
...
//        attrs.frame = CGRectMake(x, y, w, h) ;
    //} else
     if (self.columnHeights.count > destColumn + 1
        ...
        } else if (destColumn >= 1
                   //越界判断
                   && y + h - [self.columnHeights[destColumn - 1]
                               doubleValue] < w * 0.1) {//当前cell填充和上上列的高度偏差不超过cell最大高度的10%，就和下一列对齐
                       attrs.frame = CGRectMake(x, y, w, [self.columnHeights[destColumn - 1] doubleValue] - y) ;
                       _lastFixIndex = destColumn ;
